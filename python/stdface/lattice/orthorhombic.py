...
from ..core.param_check import (
    print_val_d, print_val_c, print_val_i,
...
def orthorhombic(StdI: StdIntList) -> None:
    ...
    StdI.direct[0, 2] = print_val_d("Wz", StdI.direct[0, 2], 0.0)
    StdI.direct[1, 0] = print_val_d("Lx", StdI.direct[1, 0], 0.0)
    StdI.direct[1, 1] = print_val_d("Ly", StdI.direct[1, 1], StdI.length[1])
    StdI.direct[1, 2] = print_val_d("Lz", StdI.direct[1, 2], 0.0)
    StdI.direct[2, 0] = print_val_d("Hx", StdI.direct[2, 0], 0.0)
    StdI.direct[2, 1] = print_val_d("Hy", StdI.direct[2, 1], 0.0)
    StdI.direct[2, 2] = print_val_d("Hz", StdI.direct[2, 2], StdI.length[2])
    ...
    if StdI.model == ModelType.SPIN:
        ...
        input_spin(StdI.Jpp, StdI.JppAll, "J''")
        ...
        StdI.tpp = print_val_c("tpp", StdI.tpp, 0.0)
        ...
        StdI.Vpp = print_val_d("Vpp", StdI.Vpp, 0.0)
    ...
    #     nn=3, nnn=6, nnnn=4 → n_bonds=13
    ntransMax, nintrMax = compute_max_interactions(StdI, n_bonds=3 + 6 + 4)
...
_orthorhombic_setup = orthorhombic


class OrthorhombicPlugin(LatticePlugin):
    """Plugin for the 3D simple orthorhombic lattice."""
    ...
    def aliases(self) -> list[str]:
        return ["orthorhombic", "simpleorthorhombic", "cubic", "simplecubic"]
    ...
    def setup(self, StdI: StdIntList) -> None:
        """Delegate to the orthorhombic() function."""
        _orthorhombic_setup(StdI)


register_lattice(OrthorhombicPlugin())
