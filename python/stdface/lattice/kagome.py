...
def kagome(StdI: StdIntList) -> "GnuplotData | None":
...
_kagome_setup = kagome
_kagome_boost = kagome_boost


class KagomePlugin(LatticePlugin):
    """Plugin for the 2D kagome lattice."""
    ...
    def setup(self, StdI: StdIntList) -> "GnuplotData | None":
        """Delegate to the kagome() function."""
        return _kagome_setup(StdI)

    def boost(self, StdI: StdIntList) -> None:
        """Delegate to the kagome_boost() function."""
        _kagome_boost(StdI)


register_lattice(KagomePlugin())
