...
def orthorhombic(StdI: StdIntList) -> None:
    ...
    if StdI.model == "spin":
        ...
        nintrMax = (StdI.NCell * (StdI.NsiteUC + 3 + 6 + 4)
                    * (3 * StdI.S2 + 1) * (3 * StdI.S2 + 1))
    else:
        ntransMax = StdI.NCell * 2 * (2 * StdI.NsiteUC + 6 + 12 + 8)
        nintrMax = StdI.NCell * (StdI.NsiteUC + 4 * (3 + 6 + 4))
        if StdI.model == "kondo":
    ...
    for kCell in range(StdI.NCell):
        ...
        if StdI.model == "spin":
            general_j(StdI, StdI.Jpp, StdI.S2, StdI.S2, isite, jsite)
        else:
            hopping(StdI, Cphase * StdI.tpp, isite, jsite, dR)
            coulomb(StdI, StdI.Vpp, isite, jsite)
